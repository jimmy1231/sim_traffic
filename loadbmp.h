...
#include <cstdio> /* fopen(), fwrite(), fread(), fclose() */
        ...
        const char *filename, unsigned char **imageData, unsigned int *width, unsigned int *height, unsigned int components)
{
    ...
    // Getting width and height from bmp InfoHeader (4 bytes for width, 4 bytes for header)
    // note: w, h are in terms of pixels, NOT bytes
    ...
    if ((w > 0) && (h > 0))
    {
        // components = 3 (RBG), e.g. 10x10 24-bit (RBG) bmp image, sizeof(data) = 300 bytes
        ...
        // row-order (y = row since it is the height), starts with y = (h - 1)
        for (y = (h - 1); y != -1; y--)
        {
            // for each column (starts with 0)
            for (x = 0; x < w; x++)
            {
                i = (x + y * w) * components;

                // read 1 block of 3 bytes (R, B, G)
